...
def generate_cwr(tracks: list, catalog_config: dict, agreement_map: dict,
                 sequence_number: int = 1, starting_swn: int = 1) -> tuple:
    """
    ...
    last_swn_used = starting_swn + len(tracks) - 1
...
def _lookup_agreement(pub_name: str, agreement_map: dict) -> str:
    """3-pass normalised match: exact, key-in-name, name-in-key."""
    def norm(s):
...
def _fmt_soc(value) -> str:
    """Format society code. Handles names (BMI, PRS) and numeric codes."""
    import re
    SOCIETY_LOOKUP = {
